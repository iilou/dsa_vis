...
function Tree(){
    ...
    useEffect(() => {
        ...
        let nd = [
            new Node(400, 100, nodeSize),
            new Node(200, 200, nodeSize),
            new Node(600, 200, nodeSize),
            new Node(100, 300, nodeSize),
            new Node(300, 300, nodeSize),
            new Node(500, 300, nodeSize),
            new Node(700, 300, nodeSize),
            new Node(50, 400, nodeSize),
            new Node(150, 400, nodeSize),
            new Node(250, 400, nodeSize),
            new Node(350, 400, nodeSize),
            new Node(450, 400, nodeSize),
            new Node(550, 400, nodeSize),
            new Node(650, 400, nodeSize),
            new Node(750, 400, nodeSize),
        ];
    ...
    function handleMouseMove(e){
        ...
        let inView = e.clientX >= viewDivRect.left && e.clientX <= viewDivRect.right && e.clientY >= viewDivRect.top && e.clientY <= viewDivRect.bottom;

        // if left mouse button is moved
        if(mouseDown[0] && originClicked[0] && editable){
            if(!inView) handleMouseUp({button: 0, clientX: e.clientX, clientY: e.clientY}, false);
            else {
                for(let i = 0; i < nodes.length; i++){ 
                    if(nodes[i].state == 2){ 
        ...
        if(mouseDown[2] && originClicked[2] && edgeNodeClicked != -1 && editable){
            if(!inView) handleMouseUp({button: 2, clientX: e.clientX, clientY: e.clientY}, false);
            else {
                let maintain = true;
                let switchInd = -1;
                for(let i = 0; i < nodes.length; i++){
    ...
    function handleKeyDown(e){
        ...
        if((e.key === "x" || e.key === "Delete") && mouseDown[0] === false && mouseDown[2] === false && renamingNode == -1){
            console.log(e.key);
            let found = false;
            for(let i = 0; i < nodes.length; i++){ // delete node
            ...
            if(!found) for(let i = 0; i < edges.length; i++){   // delete edge
                if(edges[i].state == 1){
                    if(edges[i].node2.id in edges[i].node1.edges){ // delete edges list
    ...
    return (
        <div className="tree" onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove} onKeyDown={handleKeyDown} onKeyUp={handleKeyUp} tabIndex={0} onContextMenu={(e) => e.preventDefault()}>
            <Header title="DSA Visualization | Tree Algorithms"/>
            <div className="treeMenu"> COMING SOON </div>
