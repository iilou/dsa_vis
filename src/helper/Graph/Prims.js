class Node{
    ...
    checkMouseHover(x, y){
        if(this.state <= 1  ){
            this.state = ((x - this.x) ** 2 + (y - this.y) ** 2 <= (this.r+5) ** 2) ? 1 : 0;
        }
    }
...
class Edge{
    ...
    resetMidpointDist(){
        this.midPoint = {x: (this.node1.x + this.node2.x) / 2, y: (this.node1.y + this.node2.y) / 2};
        this.dist = Math.sqrt((this.node1.x - this.node2.x) ** 2 + (this.node1.y - this.node2.y) ** 2);
    ...
    dist_from_point(point){// line stops at the end points
        return Math.sqrt(this.dist_to_segment_squared(point, this.node1, this.node2));
    }
